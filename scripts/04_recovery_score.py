"""
TODO: 04_recovery_score.py

Phase 4: Computes LD1, CWS, Mahalanobis Distance, or Supervised Regression Residuals based recovery score.

Input: 
    `outputs/PCA_results/PCA_scores.csv` (for Methods LD1, CWS, Mahalanobis Distance)
    `outputs/transformed_selected_features_method{A/B/C}.csv` (for Supervised Regression Residuals)

Expects folder called "outputs" with input csvs (run 01_extract_features.py then 02_transform_select_features.py)
"""